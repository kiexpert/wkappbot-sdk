...
public sealed partial class UiaLocator : IDisposable
{
    ...
    /// </summary>
    /// <summary>Get both the raw AutomationElement and its info at a screen point.</summary>
    public (AutomationElement? element, ElementAtPointInfo? info) GetElementAndInfoAtPoint(int screenX, int screenY)
    {
        try
        {
            ...
            var info = BuildElementAtPointInfo(element);
            return (element, info);
        }
    ...
    private ElementAtPointInfo BuildElementAtPointInfo(AutomationElement element)
    {
        ...
        string? value = null;
        try { if (element.Patterns.Value.IsSupported) value = element.Patterns.Value.Pattern.Value.Value; } catch { }
        var patterns = new List<string>();
        try { if (element.Patterns.Invoke.IsSupported) patterns.Add("Invoke"); } catch { }
        try { if (element.Patterns.Value.IsSupported) patterns.Add("Value"); } catch { }
        try { if (element.Patterns.Toggle.IsSupported) patterns.Add("Toggle"); } catch { }
        ...
        {
            Name = name, AutomationId = aid, ControlType = ctStr, Value = value, Patterns = patterns,
            BoundsX = rect.X, BoundsY = rect.Y, BoundsW = rect.Width, BoundsH = rect.Height,
        };
    }
    ...
    private void DumpElement(AutomationElement element, System.Text.StringBuilder sb, int depth, int maxDepth, int siblingIndex = 0)
    {
        ...
        {
            // At max depth: collect innerText summary from all descendants
            try
            {
                var leaves = new List<(string text, int x, int y, int w, int h, int depth)>();
                CollectTextLeaves(element, leaves, 0, 8);
                if (leaves.Count > 0)
                {
