...
internal class Program
{
    ...
    {
        string? formPath = args.Length > 0 ? args[0] : null;
        int iterations = int.TryParse(GetArgValue(args, "-n"), out var n) ? n : 100;
        ...
        if (string.IsNullOrEmpty(formPath))
            return Error("Usage: appbot hts-stress <form.xmf> [-n 100] [--open-ms 1000] [--close-ms 1000]");

        var hMain = WindowFinder.FindMDIMainFrameByProcessName("HTSRun");
        if (hMain == IntPtr.Zero)
            return Error("HTSRun.exe not found or no MDI frame");

        Console.WriteLine($"Target: HTSRun MDI Frame (0x{hMain:X8})");
        Console.WriteLine($"Form: {formPath}");
        Console.WriteLine($"Iterations: {iterations}, Open: {openMs}ms, Close: {closeMs}ms");
        Console.WriteLine(new string('=', 50));

        HtsInterop.StressTest(hMain, formPath, iterations, openMs, closeMs,
            (i, msg) =>
            {
                var ts = DateTime.Now.ToString("HH:mm:ss");
                Console.WriteLine($"[{ts}] #{i,4} {msg}");
            });

        Console.WriteLine(new string('=', 50));
        Console.WriteLine($"Done! {iterations} iterations complete.");
...
Usage:
  ...
  appbot hts-stress <form.xmf> [-n 100] [--open-ms 1000] [--close-ms 1000]
...
Commands:
  ...
  hts-stress HTS form open/close stress test (leak testing)
