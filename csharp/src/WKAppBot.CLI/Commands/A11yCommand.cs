...
internal partial class Program
{
    ...
    static int A11yCommand(string[] args)
    {
        if (args.Length < 2)
        {
            ...
            Console.WriteLine("  read         Read element state (TTS friendly)");
            Console.WriteLine("  find         List Win32 child windows + UIA children tree (--depth N)");
            Console.WriteLine("  highlight    Show zoom/highlight overlay on element");
            Console.WriteLine("  invoke       Invoke/click (UIA Invoke -> BM_CLICK -> WM_LBUTTON)");
            Console.WriteLine("  click        Physical click at element center");
            Console.WriteLine("  toggle       Toggle checkbox/button");
            Console.WriteLine("  expand       Expand tree/combo");
            Console.WriteLine("  collapse     Collapse tree/combo");
            Console.WriteLine("  select       Select list/tab item");
            Console.WriteLine("  scroll       Scroll (--direction up|down|left|right) (--amount small|large)");
            Console.WriteLine("  type         Type text (--text \"...\")");
            Console.WriteLine("  set-value    Set value (--text \"...\")");
            Console.WriteLine("  set-range    Set range (--value N)");
            Console.WriteLine();
            Console.WriteLine("Options:");
            Console.WriteLine("  --nth N     Target Nth match (1-based): 3, 3~, ~3, 2~4");
            Console.WriteLine("  --all       Apply to all matches");
            Console.WriteLine("  --depth N   Tree depth for find (default: 3)");
            Console.WriteLine("  --force     close: kill process if WM_CLOSE fails");
            Console.WriteLine("  --force-close-ancestors  include own process tree");
            Console.WriteLine();
            Console.WriteLine("Examples:");
            Console.WriteLine("  wkappbot a11y close \"*메모장*\" --nth 2~");
            Console.WriteLine("  wkappbot a11y find \"*메모장*\" --depth 5");
            Console.WriteLine("  wkappbot a11y highlight \"*메모장*#*파일*\"");
            Console.WriteLine("  wkappbot a11y invoke \"*메모장*#*파일*\"");
