...
internal partial class Program
{
    ...
    private static void PrintNodeAfter(AutomationElement el, NodeState before, string action, bool ok, long ms)
    {
        ...
        Console.Error.WriteLine(sep);
        Console.Error.WriteLine($"[NODE:AFTER]  action={action}  result={(ok ? "OK v" : "FAIL X")}  elapsed={ms}ms");
        Console.ResetColor();
        ...
        AutomationElement? actionTarget = null, IntPtr actionElHwnd = default)
    {
        ...
        if (fg != IntPtr.Zero)
        {
            ...
            if (intendedHwnd != IntPtr.Zero)
            {
                ...
                Console.Error.WriteLine($"  intended   : 0x{intendedHwnd.ToInt64():X8}  {(same ? "v 포그라운드 일치" : "X 포커스 강탈 감지!")}");
                Console.ResetColor();
            }
        }
    ...
    // -- 포맷 헬퍼 --------------------------------------------------------─
    private static string NdB(bool? v) => v switch { true => "v", false => "X", null => "?" };
