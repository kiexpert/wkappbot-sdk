    ...
    static int AskClaude(string question, bool slackReport = true, int timeoutSec = 30, bool newTab = false, List<string>? attachFiles = null, bool newSession = false, bool loopMode = false, int loopMaxSteps = 3, int loopRetry = 1, int loopMaxParallel = 7, bool triadMode = false, string? modelHint = null, bool noWait = false, string? targetTagOverride = null, string? linePrefix = null, TriadSharedContext? triadCtx = null)
    {
        ...
        var task = Task.Run(async () =>
        {
            try
            {
                ...
                // ?Ä?Ä Phase 3: Check existing turns ?Ä?Ä
                ...
                // ?Ä?Ä Phase 4: Insert text + send ?Ä?Ä
                using var chatLock = ChromeTabLock.Acquire("Claude");
                if (chatLock == null) return (false, (string?)null);

                // ?Ä?Ä CDP InputReadiness: blocker check + minimize restore + zoom + focus guard ?Ä?Ä
                ...
                // ?Ä?Ä Phase 5: Wait for response ?Ä?Ä
                ...
                while (sw.Elapsed.TotalSeconds < Math.Min(timeoutSec, 30))
                {
                    ...
                    var limitText = await cdp.EvalAsync("""
                        (() => {
                            ...
                            var keys = ['usage limit', 'rate limit', 'too many requests', '?îÏ≤≠???àÎ¨¥ Îß?, '?¨Ïö©???úÎèÑ'];
                    ...
                    var probeBase = Math.Max(preStreamingCount, preAssistantCount);
                    var probe = await cdp.ProbePromptResponseStateAsync(editorSel, "claude", probeBase);
                ...
                // ?Ä?Ä Phase 6: Poll for completion ?Ä?Ä
                    ...
                    try
                    {
                        ...
                        if (len > lastFlushedLen && text.Length > 0)
                        {
                            if (!liveHeaderPrinted)
                            {
                                ...
                                Console.WriteLine("?Ä?Ä Claude streaming ?Ä?Ä");
                                Console.ResetColor();
        ...
        {
            EnsureSlackThread("Claude", question);
            SlackPostToThread("??_Claude Î©îÏãúÏßÄ ?úÎèÑ Ï¥àÍ≥º_ ??claude.ai ?¨Ïö©???ïÏù∏ ?ÑÏöî", SlackAiName("claude", "Claude"));
        }
        else if (!ok)
        {
            EnsureSlackThread("Claude", question);
            SlackPostToThread("??_Claude ?ëÎãµ ?§Ìå®_", SlackAiName("claude", "Claude"));
        }
