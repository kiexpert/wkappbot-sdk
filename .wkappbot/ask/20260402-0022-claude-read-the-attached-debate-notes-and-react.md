...
> **Question**: Read the attached debate notes and react only to the current best synthesis: should WKAppBot do stdio decoupling now, what is the single main risk, and what exact rollback trigger would you use? Keep it short and concrete.
# Triad Debate
...
Do stdio decoupling now: YES.
The triad converged on this. All three AIs — even Gemini's more conservative "wait" position — implicitly accept that Console.* must be removed from the MCP pipe path before any subsystem flip. That's not migration prep, that's fixing a live ConDrv correctness bug.
Single main risk: A hot-swap child that still calls Console.* post-decoupling gets a null handle with no exception — silent data loss that manifests as guardian reading 0 bytes and entering a false respawn loop. The danger is it looks like a guardian bug, not a handle bug, so diagnosis is slow.
Exact rollback trigger: Guardian respawn count ≥ 3 within 60 seconds of the first post-upgrade hot-swap cycle. That's the earliest deterministic signal that distinguishes this failure from normal restart noise, and it doesn't require any new instrumentation — the counter already exists.

---
*Generated by WKAppBot ask claude — 2026-04-02 00:22:41*
