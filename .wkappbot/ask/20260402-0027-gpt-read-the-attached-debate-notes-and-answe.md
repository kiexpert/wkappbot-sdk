...
Do it now (Phase 1 only). Decouple stdio from Console.* immediately; defer subsystem flip.

Biggest risk: silent stdout/stderr loss → Guardian misreads 0 bytes → false crash → infinite respawn loop.

Rollback trigger: Guardian respawn count >3 within 60s OR MCP first handshake returns EOF/empty read.

---
*Generated by WKAppBot ask gpt — 2026-04-02 00:27:57*
