...
Do it now. stdio decoupling (Console.* → GetStdHandle/ReadFile/WriteFile on inherited handles) is safe-to-ship immediately — it fixes the active ConDrv 30s stall, requires no subsystem flip, and is a prerequisite for all later phases anyway.
Biggest risk: A hot-swap child that still calls Console.* post-decoupling gets a null handle silently — no exception, just 0-byte reads — which guardian misreads as a health-check failure and enters a false respawn loop.
Rollback trigger: Guardian respawn counter ≥ 3 within 60 seconds of the first post-upgrade hot-swap cycle.

---
*Generated by WKAppBot ask claude — 2026-04-02 00:24:51*
