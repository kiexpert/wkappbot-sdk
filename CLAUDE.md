...
- [ ] Chrome multiplication bug (recurring): 21-60 Chrome processes spawned on cdp open / ask gpt. NOT reproduced in isolated tests (both cdp open calls reused port 9740 correctly). Core FindRunningChromePortAny guard (8819ed449) may not be deployed OR environment/timing-dependent. SDK diagnostic + Placement.cs skill refs in place; Core deploy verify + Opus investigation pending. CRITICAL: blocks suggest triage when it fires.
