...
wkappbot suggest "text" [file.png] ["more text"] # 건의서 → Slack 웹훅 직접 POST + HQ 로컬 기록
wkappbot claude-usage                            # Claude Desktop 사용량 조회 (UIA, 포커스리스)
...
wkappbot a11y <action> <grap>[#uia-scope] [options]  # ★ 표준 통합 명령 (MCP 유일 도구)
  ...
  # Utility (3): clipboard, clipboard-read, clipboard-write (text/files/mixed with [file:] markers)
...
- **v3.5.1 clipboard + claude-usage + suggest + ArgParser**
  - a11y clipboard: read(CF_UNICODETEXT→CF_TEXT CP949 폴백), write(텍스트/파일/혼합 [file:] 마커)
