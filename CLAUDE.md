...
### 토큰 최적화 (리밋 완화 운영 가이드)
- **요약 우선**: 긴 로그/트리는 원문 전체 붙이지 말고 핵심 요약 + 파일 경로 + 라인 범위만 공유
- **구간 추출**: 에러 분석 시 최근 실패 구간(예: 마지막 100~200줄)만 우선 확인
- **단계 분리**: 조사(읽기) / 수정(패치) / 검증(테스트)을 분리해서 한 번에 한 가지 목표만 수행
- **중복 출력 금지**: 동일 상태 메시지는 `chat.update`로 갱신하고 새 메시지 남발 금지
- **대형 파일 안전 읽기**: 한 번에 전체 읽지 말고 필요한 섹션만 순차적으로 읽기
- **컨텍스트 슬림화**: 완료된 과거 이슈는 짧게 정리하고, 현재 작업에 필요한 사실만 유지
- **웹 자동화 디버깅**: 실패 시 screenshot + 핵심 selector 결과만 남기고 전체 DOM 덤프는 마지막 수단으로 사용
- **문서화 최소원칙**: 재발 방지에 필요한 교훈만 `CLAUDE.md`/주석에 기록 (장문 회고 지양)
...
- **Claude 프롬프트 완전 Focusless 입력+제출 완료**: MSAA 직접 vtable 호출 — 포커스 제로!
  - `put_accValue` → 텍스트 삽입 (focusless, UIA 프록시 우회)
  - `accDoDefaultAction("활성화")` → Enter 제출 (focusless, ROLE_GROUPING의 기본 동작)
  - 전체 흐름에서 SetForegroundWindow 한 번도 안 불림 — 사용자 작업 방해 제로
  - MSAA 트리: grandparent → parent(name="입력하세요") ← put_accValue + accDoDefaultAction 타겟
