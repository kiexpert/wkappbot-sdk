...
- [x] DONE (Chrome-multiplication diagnostic): added MyCdpContext.ChromeHealthCheck.cs DiagnoseExcessiveChromeProcesses() (>=5 Chrome before launch); wired into TryTrackMyCdpAccess after isCdpFamily check; rebuild 0 errors (6df38b66). Root cause = Core FindRunningChromePortAny guard 8819ed449 (possibly not deployed) -- Core verify pending
- [x] DONE (nightly-heal 2026-05-27 x3 + 2026-05-28): skill audit 3ok/54-cross-repo-expected/~600norefs each run, repo-health LITE clean, README What's New v7.3->v7.4, AGENTS/CLAUDE parity OK (commits 4750ecfc, 871bd83b, 4544a346, c0bea091)
...
- [x] nightly-heal 2026-05-28 (2nd, 5:13 cron): skill audit 3ok/59-cross-repo-expected/602norefs -- all broken refs point to Core repo (WKAppBot.CLI/WebBot/Win32, scripts/*.py) not SDK-owned; SUBSCRIBE.md "Bank transfer" pattern drift is Korean-doc (입금), shared skill, left intact; repo-health LITE clean (versions consistent v7.4.0-sdk, AGENTS/CLAUDE parity OK, no stubs); Pending compress (Chrome-investigation + nightly-heal entries grouped)