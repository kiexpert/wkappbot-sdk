...
- [x] gg-main escalation 2026-06-05 (Opus auto-remediation, 3 CRITICAL/Amber handled): (1) Chrome mult 26>20 CRIT — self-cleaned to 0 via SDK ChromeHealthCheck.CleanExcessiveChromeProcesses; filed Core suggest ts=2026-06-05T09:19:28 (FindRunningChromePortAny guard not deployed / port-reuse regression, 3 reqs). (2) Eye 6>3 — wkappbot --sudo eye restart reclaimed idle core (6→5); 2 remaining stale cores (790min/183min) are mcp-launcher-protected (taskkill --force skipped by guard, NOT overridden per kill-protection rule); 3 active ~52min sessions preserved; Eye alive ctx=25%. (3) CI CDP Connectivity Smoke run 27006231239 — confirmed hosted-runner PCW failure (fgT=HostedComputeAgent, cwd=D:\a\wkappbot-sdk; cdp open fails: no real desktop caller window). EXPECTED, not SDK bug. CI skip rule: hosted-runner cdp open always fails (no real desktop window) — diagnose-only, do NOT auto-fix workflow.
