...
if exist "%BIN_DIR%\wkappbot.exe" (
  ...
  set "AI_PROMPT=WKAppBot just built successfully! The user's terminal code page is !CODEPAGE!. Respond in the natural language that matches this code page. You are an expert WKAppBot assistant. Give a rich enthusiastic welcome: (1) explain in 2-3 sentences what WKAppBot is and why it matters for AI agents, (2) list 5 specific things the user can try RIGHT NOW with exact commands -- note: 'wkappbot windows' with no args filters by wkappbot keyword by default (use 'wkappbot windows *' for all windows), (3) highlight the Skill system: 'wkappbot skill list' shows accumulated expert knowhow like grap syntax, UIA quirks, CDP tips -- these skills are the secret weapon that makes WKAppBot self-improving, (4) share 1 hidden pro tip that surprises most new users. Use clear sections and make it exciting."
  echo.
  echo ============================================================
