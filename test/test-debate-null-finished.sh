#!/bin/bash
# test-debate-null-finished.sh — verify debate null subcommand exits cleanly (smoke test)
# Real execution: wkappbot debate null
# CMD guard: dbgCmd=debate, dbgSub=null → -debate-null>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-debate-null-finished Real Execution Test ==="

echo -n "Test 1: debate null exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" debate null >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: debate null exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" debate null >/dev/null 2>&1; CODE=$?
