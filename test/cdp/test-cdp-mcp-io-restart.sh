#!/bin/bash
# test-cdp-mcp-io-restart.sh — verify cdp mcp subcommand exits cleanly (smoke test)
# Real execution: wkappbot cdp mcp
# CMD guard: dbgCmd=cdp, dbgSub=mcp → -cdp-mcp>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-cdp-mcp-io-restart Real Execution Test ==="

echo -n "Test 1: cdp mcp exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp mcp >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: cdp mcp exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp mcp >/dev/null 2>&1; CODE=$?
