#!/bin/bash
# test-cdp-file-chooser.sh — verify cdp file subcommand exits cleanly (smoke test)
# Real execution: wkappbot cdp file
# CMD guard: dbgCmd=cdp, dbgSub=file → -cdp-file>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-cdp-file-chooser Real Execution Test ==="

echo -n "Test 1: cdp file exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp file >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: cdp file exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp file >/dev/null 2>&1; CODE=$?
