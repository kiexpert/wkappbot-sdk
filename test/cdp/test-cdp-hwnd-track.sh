#!/bin/bash
# test-cdp-hwnd-track.sh — verify cdp hwnd subcommand exits cleanly (smoke test)
# Real execution: wkappbot cdp hwnd
# CMD guard: dbgCmd=cdp, dbgSub=hwnd → -cdp-hwnd>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-cdp-hwnd-track Real Execution Test ==="

echo -n "Test 1: cdp hwnd exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp hwnd >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: cdp hwnd exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp hwnd >/dev/null 2>&1; CODE=$?
