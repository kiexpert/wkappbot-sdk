#!/bin/bash
# test-cdp-focus-noactivate.sh — verify cdp focus subcommand exits cleanly (smoke test)
# Real execution: wkappbot cdp focus
# CMD guard: dbgCmd=cdp, dbgSub=focus → -cdp-focus>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-cdp-focus-noactivate Real Execution Test ==="

echo -n "Test 1: cdp focus exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp focus >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: cdp focus exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp focus >/dev/null 2>&1; CODE=$?
