#!/bin/bash
# test-cdp-hotkey-map.sh — verify cdp hotkey subcommand exits cleanly (smoke test)
# Real execution: wkappbot cdp hotkey
# CMD guard: dbgCmd=cdp, dbgSub=hotkey → -cdp-hotkey>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-cdp-hotkey-map Real Execution Test ==="

echo -n "Test 1: cdp hotkey exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp hotkey >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: cdp hotkey exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp hotkey >/dev/null 2>&1; CODE=$?
