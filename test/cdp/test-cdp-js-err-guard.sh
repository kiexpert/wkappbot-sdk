#!/bin/bash
# test-cdp-js-err-guard.sh — verify cdp js subcommand exits cleanly (smoke test)
# Real execution: wkappbot cdp js
# CMD guard: dbgCmd=cdp, dbgSub=js → -cdp-js>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-cdp-js-err-guard Real Execution Test ==="

echo -n "Test 1: cdp js exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp js >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: cdp js exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp js >/dev/null 2>&1; CODE=$?
