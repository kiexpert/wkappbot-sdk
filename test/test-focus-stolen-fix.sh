#!/bin/bash
# test-focus-stolen-fix.sh — verify focus stolen exits cleanly
# Real execution: wkappbot focus stolen [--help]
# CMD guard: dbgCmd=focus, dbgSub=stolen → -focus-stolen>
# Note: 'focus stolen' hangs without --help (tries to interact with UI)
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-focus-stolen-fix Real Execution Test ==="

echo -n "Test 1: focus stolen exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" focus stolen --help >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: focus stolen repeat call exits without hang... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" focus stolen --help >/dev/null 2>&1; CODE=$?
