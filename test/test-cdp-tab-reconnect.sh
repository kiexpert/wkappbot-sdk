#!/bin/bash
# test-cdp-tab-reconnect.sh — verify cdp tab subcommand exits cleanly (smoke test)
# Real execution: wkappbot cdp tab
# CMD guard: dbgCmd=cdp, dbgSub=tab → -cdp-tab>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-cdp-tab-reconnect Real Execution Test ==="

echo -n "Test 1: cdp tab exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp tab >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: cdp tab exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp tab >/dev/null 2>&1; CODE=$?
