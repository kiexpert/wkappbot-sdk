#!/bin/bash
# test-focusless-sw-min.sh — focusless sw subcommand exits cleanly
# Real execution: wkappbot focusless sw
# CMD guard: dbgCmd=focusless, dbgSub=sw → -focusless-sw>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-focusless-sw-min Real Execution Test ==="

echo -n "Test 1: focusless sw exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" focusless sw >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: focusless sw exits without crash (exit code != 124)... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" focusless sw >/dev/null 2>&1; CODE=$?
