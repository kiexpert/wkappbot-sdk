#!/bin/bash
# test-cdp-input-cjk.sh — verify cdp input subcommand exits cleanly (smoke test)
# Real execution: wkappbot cdp input
# CMD guard: dbgCmd=cdp, dbgSub=input → -cdp-input>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-cdp-input-cjk Real Execution Test ==="

echo -n "Test 1: cdp input exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp input >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: cdp input exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp input >/dev/null 2>&1; CODE=$?
