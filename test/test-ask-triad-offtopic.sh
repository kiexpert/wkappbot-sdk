#!/bin/bash
# test-ask-triad-offtopic.sh — verify ask triad subcommand exits cleanly (smoke test)
# Real execution: wkappbot ask triad
# CMD guard: dbgCmd=ask, dbgSub=triad → -ask-triad>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-ask-triad-offtopic Real Execution Test ==="

echo -n "Test 1: ask triad exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" ask triad >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: ask triad exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" ask triad >/dev/null 2>&1; CODE=$?
