#!/bin/bash
# test-a11y-windows-wsl-20260330-071628.sh — verify a11y windows subcommand exits cleanly (smoke test)
# Real execution: wkappbot a11y windows
# CMD guard: dbgCmd=a11y, dbgSub=windows → -a11y-windows>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-a11y-windows-wsl-20260330-071628 Real Execution Test ==="

echo -n "Test 1: a11y windows exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" a11y windows >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: a11y windows exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" a11y windows >/dev/null 2>&1; CODE=$?
