#!/bin/bash
# test-cdp-zoom-fallback.sh — verify cdp zoom subcommand exits cleanly (smoke test)
# Real execution: wkappbot cdp zoom
# CMD guard: dbgCmd=cdp, dbgSub=zoom → -cdp-zoom>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-cdp-zoom-fallback Real Execution Test ==="

echo -n "Test 1: cdp zoom exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp zoom >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: cdp zoom exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp zoom >/dev/null 2>&1; CODE=$?
