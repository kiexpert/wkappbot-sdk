#!/bin/bash
# test-a11y-wait.sh — verify a11y wait subcommand exits cleanly (smoke test)
# Real execution: wkappbot a11y wait
# CMD guard: dbgCmd=a11y, dbgSub=wait → -a11y-wait>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-a11y-wait Real Execution Test ==="

echo -n "Test 1: a11y wait exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" a11y wait >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: a11y wait exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" a11y wait >/dev/null 2>&1; CODE=$?
