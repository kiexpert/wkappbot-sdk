#!/bin/bash
# test-cdp-localhost-tab.sh — verify cdp localhost subcommand exits cleanly (smoke test)
# Real execution: wkappbot cdp localhost
# CMD guard: dbgCmd=cdp, dbgSub=localhost → -cdp-localhost>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-cdp-localhost-tab Real Execution Test ==="

echo -n "Test 1: cdp localhost exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp localhost >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: cdp localhost exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp localhost >/dev/null 2>&1; CODE=$?
