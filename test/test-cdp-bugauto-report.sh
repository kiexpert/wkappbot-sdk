#!/bin/bash
# test-cdp-bugauto-report.sh — verify cdp bugauto subcommand exits cleanly (smoke test)
# Real execution: wkappbot cdp bugauto
# CMD guard: dbgCmd=cdp, dbgSub=bugauto → -cdp-bugauto>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-cdp-bugauto-report Real Execution Test ==="

echo -n "Test 1: cdp bugauto exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp bugauto >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: cdp bugauto exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp bugauto >/dev/null 2>&1; CODE=$?
