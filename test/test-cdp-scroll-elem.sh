#!/bin/bash
# test-cdp-scroll-elem.sh — verify cdp scroll subcommand exits cleanly (smoke test)
# Real execution: wkappbot cdp scroll
# CMD guard: dbgCmd=cdp, dbgSub=scroll → -cdp-scroll>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-cdp-scroll-elem Real Execution Test ==="

echo -n "Test 1: cdp scroll exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp scroll >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: cdp scroll exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp scroll >/dev/null 2>&1; CODE=$?
