#!/bin/bash
# test-zoom-showpass-race.sh — verify zoom showpass exits cleanly
# Real execution: wkappbot zoom showpass [--help]
# CMD guard: dbgCmd=zoom, dbgSub=showpass → -zoom-showpass>
# Note: 'zoom showpass' exits quickly (unknown subcommand)
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-zoom-showpass-race Real Execution Test ==="

echo -n "Test 1: zoom showpass exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" zoom showpass >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: zoom showpass repeat call exits without hang... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" zoom showpass >/dev/null 2>&1; CODE=$?
