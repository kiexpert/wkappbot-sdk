#!/bin/bash
# test-cdp-eval-retry.sh — verify cdp eval subcommand exits cleanly (smoke test)
# Real execution: wkappbot cdp eval
# CMD guard: dbgCmd=cdp, dbgSub=eval → -cdp-eval>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-cdp-eval-retry Real Execution Test ==="

echo -n "Test 1: cdp eval exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp eval >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: cdp eval exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp eval >/dev/null 2>&1; CODE=$?
