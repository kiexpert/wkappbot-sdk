#!/bin/bash
# test-debate-emoji-fixed.sh — verify debate emoji subcommand exits cleanly (smoke test)
# Real execution: wkappbot debate emoji
# CMD guard: dbgCmd=debate, dbgSub=emoji → -debate-emoji>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-debate-emoji-fixed Real Execution Test ==="

echo -n "Test 1: debate emoji exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" debate emoji >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: debate emoji exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" debate emoji >/dev/null 2>&1; CODE=$?
