#!/bin/bash
# test-triad-tool-share.sh — triad tool subcommand exits cleanly
# Real execution: wkappbot triad tool
# CMD guard: dbgCmd=triad, dbgSub=tool → -triad-tool>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-triad-tool-share Real Execution Test ==="

echo -n "Test 1: triad tool exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" triad tool >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: triad tool exits without crash (exit code != 124)... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" triad tool >/dev/null 2>&1; CODE=$?
