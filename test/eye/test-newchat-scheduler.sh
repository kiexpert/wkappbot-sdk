#!/bin/bash
# test-newchat-scheduler.sh — newchat scheduler subcommand exits cleanly
# Real execution: wkappbot newchat scheduler
# CMD guard: dbgCmd=newchat, dbgSub=scheduler → -newchat-scheduler>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-newchat-scheduler Real Execution Test ==="

echo -n "Test 1: newchat scheduler exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" newchat scheduler >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: newchat scheduler exits without crash (exit code != 124)... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" newchat scheduler >/dev/null 2>&1; CODE=$?
