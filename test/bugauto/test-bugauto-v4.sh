#!/bin/bash
# test-bugauto-v4.sh — verify bugauto v4 subcommand exits cleanly (smoke test)
# Real execution: wkappbot bugauto v4
# CMD guard: dbgCmd=bugauto, dbgSub=v4 → -bugauto-v4>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-bugauto-v4 Real Execution Test ==="

echo -n "Test 1: bugauto v4 exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" bugauto v4 >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: bugauto v4 exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" bugauto v4 >/dev/null 2>&1; CODE=$?
