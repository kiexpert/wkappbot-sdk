#!/bin/bash
# test-flutter-homework.sh — verify flutter homework subcommand exits cleanly (smoke test)
# Real execution: wkappbot flutter homework
# CMD guard: dbgCmd=flutter, dbgSub=homework → -flutter-homework>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-flutter-homework Real Execution Test ==="

echo -n "Test 1: flutter homework exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" flutter homework >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: flutter homework exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" flutter homework >/dev/null 2>&1; CODE=$?
