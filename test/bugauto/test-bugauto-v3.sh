#!/bin/bash
# test-bugauto-v3.sh — verify bugauto v3 subcommand exits cleanly (smoke test)
# Real execution: wkappbot bugauto v3
# CMD guard: dbgCmd=bugauto, dbgSub=v3 → -bugauto-v3>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-bugauto-v3 Real Execution Test ==="

echo -n "Test 1: bugauto v3 exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" bugauto v3 >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: bugauto v3 exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" bugauto v3 >/dev/null 2>&1; CODE=$?
