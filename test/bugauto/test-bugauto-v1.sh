#!/bin/bash
# test-bugauto-v1.sh — verify bugauto v1 subcommand exits cleanly (smoke test)
# Real execution: wkappbot bugauto v1
# CMD guard: dbgCmd=bugauto, dbgSub=v1 → -bugauto-v1>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-bugauto-v1 Real Execution Test ==="

echo -n "Test 1: bugauto v1 exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" bugauto v1 >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: bugauto v1 exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" bugauto v1 >/dev/null 2>&1; CODE=$?
