#!/bin/bash
# test-cdp-mouse-actions.sh — verify cdp mouse subcommand exits cleanly (smoke test)
# Real execution: wkappbot cdp mouse
# CMD guard: dbgCmd=cdp, dbgSub=mouse → -cdp-mouse>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-cdp-mouse-actions Real Execution Test ==="

echo -n "Test 1: cdp mouse exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp mouse >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: cdp mouse exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp mouse >/dev/null 2>&1; CODE=$?
