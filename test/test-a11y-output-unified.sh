#!/bin/bash
# test-a11y-output-unified.sh — verify a11y output subcommand exits cleanly (smoke test)
# Real execution: wkappbot a11y output
# CMD guard: dbgCmd=a11y, dbgSub=output → -a11y-output>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-a11y-output-unified Real Execution Test ==="

echo -n "Test 1: a11y output exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" a11y output >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: a11y output exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" a11y output >/dev/null 2>&1; CODE=$?
