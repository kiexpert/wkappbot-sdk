#!/bin/bash
# test-debate-reminder-args.sh — verify debate reminder subcommand exits cleanly (smoke test)
# Real execution: wkappbot debate reminder
# CMD guard: dbgCmd=debate, dbgSub=reminder → -debate-reminder>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-debate-reminder-args Real Execution Test ==="

echo -n "Test 1: debate reminder exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" debate reminder >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: debate reminder exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" debate reminder >/dev/null 2>&1; CODE=$?
