#!/bin/bash
# test-webbot-target.sh — verify webbot target subcommand exits cleanly (smoke test)
# Real execution: wkappbot webbot target
# CMD guard: dbgCmd=webbot, dbgSub=target → -webbot-target>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-webbot-target Real Execution Test ==="

echo -n "Test 1: webbot target exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" webbot target >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: webbot target exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" webbot target >/dev/null 2>&1; CODE=$?
