#!/bin/bash
# test-windows-focus-path.sh — verify windows focus exits cleanly
# Real execution: wkappbot windows focus [--help]
# CMD guard: dbgCmd=windows, dbgSub=focus → -windows-focus>
# Note: 'windows focus' hangs without --help (tries to interact with UI)
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-windows-focus-path Real Execution Test ==="

echo -n "Test 1: windows focus exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" windows focus --help >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: windows focus repeat call exits without hang... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" windows focus --help >/dev/null 2>&1; CODE=$?
