#!/bin/bash
# test-a11y-pulsestep-uia.sh — verify a11y pulsestep subcommand exits cleanly (smoke test)
# Real execution: wkappbot a11y pulsestep
# CMD guard: dbgCmd=a11y, dbgSub=pulsestep → -a11y-pulsestep>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-a11y-pulsestep-uia Real Execution Test ==="

echo -n "Test 1: a11y pulsestep exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" a11y pulsestep >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: a11y pulsestep exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" a11y pulsestep >/dev/null 2>&1; CODE=$?
