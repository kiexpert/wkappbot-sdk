#!/bin/bash
# test-ask-cdp-minimized.sh — verify ask cdp subcommand exits cleanly (smoke test)
# Real execution: wkappbot ask cdp
# CMD guard: dbgCmd=ask, dbgSub=cdp → -ask-cdp>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-ask-cdp-minimized Real Execution Test ==="

echo -n "Test 1: ask cdp exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" ask cdp >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: ask cdp exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" ask cdp >/dev/null 2>&1; CODE=$?
