#!/bin/bash
# test-cdp-ai-helpers.sh — verify cdp ai subcommand exits cleanly (smoke test)
# Real execution: wkappbot cdp ai
# CMD guard: dbgCmd=cdp, dbgSub=ai → -cdp-ai>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-cdp-ai-helpers Real Execution Test ==="

echo -n "Test 1: cdp ai exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp ai >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: cdp ai exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" cdp ai >/dev/null 2>&1; CODE=$?
