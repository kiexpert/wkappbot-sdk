#!/bin/bash
# test-a11y-coldstart-profile.sh — verify a11y coldstart subcommand exits cleanly (smoke test)
# Real execution: wkappbot a11y coldstart
# CMD guard: dbgCmd=a11y, dbgSub=coldstart → -a11y-coldstart>
PASS=0; FAIL=0
WKBOT="${WKBOT:-/w/SDK/bin/wkappbot.exe}"
echo "=== test-a11y-coldstart-profile Real Execution Test ==="

echo -n "Test 1: a11y coldstart exits in < 5s... "
START=$(date +%s%3N)
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" a11y coldstart >/dev/null 2>&1; CODE=$?
END=$(date +%s%3N)
ELAPSED=$((END - START))
...
echo -n "Test 2: a11y coldstart exits without crash... "
WKAPPBOT_WORKER=1 timeout 8 "$WKBOT" a11y coldstart >/dev/null 2>&1; CODE=$?
